...
// 🔥 FUNCIONES RANDOM (las guardamos para usarlas luego)
settings.getRandomChannel = () => settings.channels[Math.floor(Math.random() * settings.channels.length)]
settings.getRandomBanner = () => settings.banners[Math.floor(Math.random() * settings.banners.length)]
settings.getRandomIcon = () => settings.icons[Math.floor(Math.random() * settings.icons.length)]
