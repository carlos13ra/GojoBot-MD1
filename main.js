...
export default async (client, m) => {
  ...
  const isBotAdmins = m.isGroup
    ? groupAdmins.some((p) => p.phoneNumber === botJid || p.jid === botJid || p.id === botJid || p.lid === botJid)
    : false;
  const isAdmins = m.isGroup
    ? groupAdmins.some((p) => p.phoneNumber === sender || p.jid === sender || p.id === sender || p.lid === sender)
    : false;
  ...
  // =========================
  // Validaciones de permisos y owner
  // =========================
  ...
  // =========================
  if (
    (m.id.startsWith('3EB0') || (m.id.startsWith('BAE5') && m.id.length === 16) || (m.id.startsWith('B24E') && m.id.length === 20))
  )
    return;
  ...
  const today = new Date()
    .toLocaleDateString('es-CO', { timeZone: 'America/Bogota', year: 'numeric', month: '2-digit', day: '2-digit' })
    .split('/')
    .reverse()
    .join('-');
