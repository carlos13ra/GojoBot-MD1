...
export default {
  command: ['play', 'mp3', 'playaudio'],
  category: 'downloader',
  run: async (client, m, args, usedPrefix, command) => {
    try {
      if (!args[0]) {
        return m.reply('᯽︎ Por favor, menciona el nombre o URL del video que deseas descargar')
      }
...
async function getAudioFromApis(url) {
  const apis = [
    { api: 'Nexuz Light', endpoint: `${api.url}/download/ytaudio?url=${encodeURIComponent(url)}`, extractor: res => res?.data?.download },
